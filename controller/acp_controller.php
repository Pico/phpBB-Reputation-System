	...
	*/
	public function manage_options()
	{
		...
		$display_vars = array(
			...
			'rs_user_rating'		=> array('lang' => 'RS_USER_RATING', 'validate' => 'bool', 'type' => 'custom', 'function' => array($method, 'user_rating'), 'explain' => false),
			...
			'rs_enable_comment'		=> array('lang' => 'RS_ENABLE_COMMENT', 'validate' => 'bool', 'type' => 'custom', 'function' => array($method, 'comment'), 'explain' => true),
			...
			'rs_enable_power'		=> array('lang' => 'RS_ENABLE_POWER', 'validate' => 'bool', 'type' => 'custom', 'function' => array($method, 'power'), 'explain' => true),
			...
			'rs_enable_toplist'		=> array('lang' => 'RS_ENABLE_TOPLIST', 'validate' => 'bool', 'type' => 'custom', 'function' => array($method, 'toplist'), 'explain' => true),
		...
		foreach ($display_vars as $config_key => $vars)
		{
			...
			if (strpos($config_key, 'section') !== false)
			{
				$this->template->assign_block_vars('options', array(
					'S_SECTION'		=> true,
					'LEGEND'		=> isset($vars['lang']) ? $this->user->lang($vars['lang']) : '',
				));
...
class setting_methods
{
	...
	*/
	...
	/**
	* Build modified radio fields for acp reputation page
	* It allows using tabs
	*/
	function m_radio($name, $input_ary, $input_default = false, $id = false, $key = false, $onclick = false, $separator = '')
	{
		$html = '';
		$id_assigned = false;
		...
		foreach ($input_ary as $value => $title)
		{
			...
			$html .= '<label><input type="radio" name="' . $name . '"' . (($id && !$id_assigned) ? ' id="' . $id . '"' : '') . (($onclick) ? ' onclick="' . $onclick . ';"' : '') . ' value="' . $value . '"' . $selected . (($key) ? ' accesskey="' . $key . '"' : '') . ' class="radio" /> ' . $this->user->lang($title) . '</label>' . $separator;
			$id_assigned = true;
		}
	...
	*/
	function post_rating($value, $key)
	{
		...
		$option = $this->m_radio('config[rs_post_rating]', $radio_ary, $value, 'post_rating', $key, "init_check('post_rating')");
	...
	/**
	* Enable/disable user rating
	*/
	function user_rating($value, $key)
	{
		$radio_ary = array(1 => 'YES', 0 => 'NO');

		$option = $this->m_radio('config[rs_user_rating]', $radio_ary, $value, 'user_rating', $key, 'init_check(\'user_rating\')');

		return $option;
	}

	/**
	* Enable/disable reputation comment
	*/
	function comment($value, $key)
	{
		$radio_ary = array(1 => 'YES', 0 => 'NO');

		$option = $this->m_radio('config[rs_enable_comment]', $radio_ary, $value, 'comment', $key, 'init_check(\'comment\')');

		return $option;
	}

	/**
	* Enable/disable reputation power
	*/
	function power($value, $key)
	{
		$radio_ary = array(1 => 'YES', 0 => 'NO');

		$option = $this->m_radio('config[rs_enable_power]', $radio_ary, $value, 'power', $key, 'init_check(\'power\')');

		return $option;
	}

	/**
	* Enable/disable reputation toplist
	*/
	function toplist($value, $key)
	{
		$radio_ary = array(1 => 'YES', 0 => 'NO');

		$option = $this->m_radio('config[rs_enable_toplist]', $radio_ary, $value, 'toplist', $key, 'init_check(\'toplist\')');
